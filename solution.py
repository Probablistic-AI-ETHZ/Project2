...
def main():
    ...
    training_images = torch.from_numpy(
        np.load(data_location / "train_xs.npz")["train_xs"]
    )
    ...
    training_dataset = torch.utils.data.TensorDataset(
        training_images, training_snow_labels, training_cloud_labels, training_labels
    )

    # Load validation data
    validation_images = torch.from_numpy(
        np.load(data_location / "val_xs.npz")["val_xs"]
    )
    ...
    validation_dataset = torch.utils.data.TensorDataset(
        validation_images,
        validation_snow_labels,
        validation_cloud_labels,
        validation_labels,
    )
    ...
    with torch.random.fork_rng():
        evaluate(
            swag_inference, validation_dataset, EXTENDED_EVALUATION, output_location
        )
...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self._swag_squared_mean_weights = self._create_weight_copy()
        ...
        self._swag_mean_weights = self._create_weight_copy()
        self._swag_squared_mean_weights = self._create_weight_copy()
        self._swag_deviation_matrices = collections.deque(
            maxlen=self.max_rank_deviation_matrix
        )
    ...
    def update_swag_statistics(self) -> None:
        ...
        for name, param in copied_params.items():
            ...
            if self._swag_num_samples == 0:
                self._swag_mean_weights[name] = param.clone()
                self._swag_squared_mean_weights[name] = param.clone() ** 2
            else:
                self._swag_mean_weights[name] += param
                self._swag_squared_mean_weights[name] += param**2
        ...
        if self.inference_mode == InferenceType.SWAG_FULL:
            ...
            deviation = {
                name: param - self._swag_mean_weights[name] / self._swag_num_samples
                for name, param in copied_params.items()
            }
            self._swag_deviation_matrices.append(deviation)
            self._swag_full_num_samples += 1
    ...
    def fit_swag_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        for name, param in self.network.named_parameters():
            self._swag_mean_weights[name].zero_()
            self._swag_squared_mean_weights[name].zero_()

        self.network.train()
        with tqdm.trange(
            self.swag_training_epochs, desc="Running gradient descent for SWA"
        ) as pbar:
            progress_dict = {}
            for epoch in pbar:
                avg_loss = 0.0
                ...
                for (
                    ...
                    avg_loss = (
                        batch_images.size(0) * batch_loss.item()
                        + num_samples * avg_loss
    ...
    def apply_calibration(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        val_images, val_snow_labels, val_cloud_labels, val_labels = (
            validation_data.tensors
        )
    ...
    def predict_probabilities_swag(
        ...
        for _ in tqdm.trange(
            self.num_bma_samples, desc="Performing Bayesian model averaging"
        ):
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            # mean_weights = ...
            # std_weights = ...
            mean_weights = self._swag_mean_weights[name] / self._swag_num_samples
            variance = (
                self._swag_squared_mean_weights[name] / self._swag_num_samples
                - mean_weights**2
            ).clamp(min=1e-30)
            std_weights = torch.sqrt(variance)
            assert (
                mean_weights.size() == param.size()
                and std_weights.size() == param.size()
            )
            ...
            if self.inference_mode == InferenceType.SWAG_FULL:
                ...
                deviations = torch.stack(
                    [dev[name].flatten() for dev in self._swag_deviation_matrices],
                    dim=0,
                )
                rank = deviations.size(0)
                z_full = torch.randn(rank).to(param.device)
                full_covariance_contribution = deviations.t().matmul(
                    z_full / (rank - 1) ** 0.5
                )
                sampled_weight += full_covariance_contribution.view(param.size())
    ...
    def fit(
        ...
        if self.inference_mode in (
            InferenceType.SWAG_DIAGONAL,
            InferenceType.SWAG_FULL,
        ):
    ...
    def fit_map_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self.network.train()
        with tqdm.trange(
            map_training_epochs, desc="Fitting initial MAP weights"
        ) as pbar:
            progress_dict = {}
...
def evaluate(
    ...
    max_pred_probabilities, argmax_pred_labels = torch.max(
        all_pred_probabilities, dim=-1
    )
    ...
    non_ambiguous_accuracy = torch.mean(
        (predicted_labels[non_ambiguous_mask] == labels[non_ambiguous_mask]).float()
    ).item()
    ...
    print(
        f"Accuracy (non-ambiguous only, your predictions): {non_ambiguous_accuracy:.4f}"
    )
    print(
        f"Accuracy (non-ambiguous only, predicting most-likely class): {non_ambiguous_argmax_accuracy:.4f}"
    )
    ...
    for threshold in threshold_values:
        thresholded_predictions = torch.where(
            max_pred_probabilities <= threshold,
            -1 * torch.ones_like(predicted_labels),
            predicted_labels,
        )
        costs.append(cost_function(thresholded_predictions, labels).item())
    best_threshold_index = np.argmin(costs)
    print(
        f"Best cost {costs[best_threshold_index]} at threshold {threshold_values[best_threshold_index]}"
    )
    ...
    calibration_data = calc_calibration_curve(
        all_pred_probabilities.numpy(), labels.numpy(), num_bins=20
    )
        ...
        for row in range(0, 4, 2):
            for col in range(5):
                ...
                ax[row + 1, col].set_title(
                    f"pred. {predicted_labels[sample_index]}, true {labels[sample_index]}"
                )
                bar_colors = ["C0"] * 6
